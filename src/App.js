...
// V29 UPDATE: Added logo to AuthPage and global footer "Evergreen Tots 2025 © Evergreen Tots App".
// Assumes logo file in public folder is named 'evergreen-logo-2.png'.
...
import {
    ...
    ArrowLeft, DollarSign, BookOpen, Image as ImageIcon, Paperclip, AlertTriangle, ListOrdered, Briefcase, UploadCloud, FilePlus, UserCheck, UserX as UserXIcon,
...
const uploadFileToSupabase = async (file, bucketName, pathPrefix = '') => {
    ...
    try {
        ...
        console.log('File uploaded to Supabase Storage, attempting to get public URL for:', fileName);
        ...
        if (!publicUrlData || !publicUrlData.publicUrl) {
            ...
            throw new Error('Could not get public URL for uploaded file. Check bucket policies and file path. Public URL data was empty.');
        }
...
const uploadReportPhoto = async (file, childId, photoNumber) => {
    ...
    console.log(`Attempting to upload report photo to: ${BUCKET_NAME} with prefix: ${pathPrefix}`);
    try {
...
const TeacherDashboardPage = () => {
    ...
    return (
        <div className="page-card admin-dashboard"> {/* Re-use admin-dashboard for consistent section titles */}
    ...
    <div className="auth-page-container"> 
      <div className="auth-card"> 
        ...
        <div className="auth-logo-wrapper" style={{ textAlign: 'center', marginBottom: '20px' }}>
            <img 
                src="/evergreen-logo-2.png" // Ensure this file is in your /public folder and named correctly
        ...
        <div className="auth-header"> 
          {/* Original icon wrapper removed as logo is now above */}
...
const ViewDailyReportModal = ({ report, child, staff, onClose }) => {
    ...
    // Find the specific staff member who created the report
                ...
                <div className="gallery-grid">
                    {photos.map(photo => (
                        <div key={photo.id} className="gallery-item-card"> 
                            <img 
                                src={photo.url} 
                                alt={photo.caption || `Photo for ${photo.childName}`} 
...
const App = () => {
  ...
  const [loggedInStaffProfile, setLoggedInStaffProfile] = useState(null); 
  ...
  const showAlert = (message, type = 'success') => { console.log(`ALERT (${type}): ${message}`); /* Replace with a proper UI notification system */ alert(`${type.toUpperCase()}: ${message}`); };
  ...
  useEffect(() => {
    ...
    const processSessionChange = async (sessionData, eventType = null) => {
        ...
        if (sessionData?.user) {
            ...
            if (['admin', 'teacher', 'assistant'].includes(userProfileDetails.role) && userProfileDetails.staff_id) {
                try {
                    const { data: fullStaffProfile, error: fullStaffError } = await supabase
                        .from('staff')
                        .select('*')
                        .eq('id', userProfileDetails.staff_id) 
                        .single();
                    if (fullStaffError && fullStaffError.code !== 'PGRST116') {
                        console.error("Error fetching full staff profile for loggedInStaffProfile:", fullStaffError);
                        setLoggedInStaffProfile(null);
                    } else {
                        setLoggedInStaffProfile(fullStaffProfile || null);
                    }
                } catch (e) {
                    console.error("Exception fetching full staff profile:", e);
                    setLoggedInStaffProfile(null);
                }
            } else {
                setLoggedInStaffProfile(null);
  ...
  useEffect(() => {
    ...
    if (session && supabase?.channel && appMode && appMode !== 'auth' && !['unknown', 'unknown_profile', 'no_parent_profile', 'exception_profile'].includes(appMode)) {
      ...
      mainChannel = supabase.channel(`app-data-updates-${appMode}-v26-realtime`); 
      tablesToSubscribe.forEach(table => {
          mainChannel.on('postgres_changes', { event: '*', schema: 'public', table: table.name }, (payload) => {
              ...
              const dataTypeKey = table.name.replace(/_([a-z])/g, (g) => g[1].toUpperCase()) + 'DataRT'; 
              fetchData(dataTypeKey, table.setter, table.name, table.order, table.select || '*');
          });
      });
  ...
  }, [supabase, showAlert, setCurrentPage]);
  ...
  const updateChildInSupabase = useCallback(async (updatedChildData) => {
    if (!supabase || !childToEdit?.id) return;
  ...
  }, [supabase, showAlert, childToEdit]);

  const deleteChildFromSupabase = useCallback(async (childId) => { 
    if (!supabase || !window.confirm("Are you sure you want to delete this child? This action cannot be undone.")) return; 
  ...
  }, [supabase, showAlert, setCurrentPage, currentUser]);
  ...
  const updateStaffInSupabase = useCallback(async (updatedStaffData) => {
    if (!supabase || !staffToEdit?.id) return;
    try {
  ...
  }, [supabase, showAlert, staffToEdit]);

  const deleteStaffFromSupabase = useCallback(async (staffId) => { 
    if (!supabase || !window.confirm("Are you sure you want to delete this staff member?")) return; 
  ...
  const updateRoomInSupabase = useCallback(async (updatedRoomData) => { 
    if (!supabase || !roomToEdit?.id) return; 
  ...
  }, [supabase, showAlert, roomToEdit]);

  const deleteRoomFromSupabase = useCallback(async (roomId) => { 
    if (!supabase || !window.confirm("Are you sure you want to delete this room?")) return; 
  ...
  }, [supabase, showAlert, currentUser, appMode, setCurrentPage]);
  ...
  }, [supabase, showAlert, currentUser, setCurrentPage]);
  ...
  const addMedicationToSupabase = useCallback(async (medData) => { if (!supabase) return; try { const { error } = await supabase.from('medications').insert([medData]).select(); if (error) throw error; showAlert('Medication added!'); setShowAddMedicationModal(false); } catch (e) { showAlert(`Add medication error: ${e.message}`, 'error'); } }, [supabase, showAlert]);
  const updateMedicationInSupabase = useCallback(async (medData) => { if (!supabase || !medData.id) return; try { const {id, ...dataToUpdate} = medData; const { error } = await supabase.from('medications').update(dataToUpdate).eq('id', id).select(); if (error) throw error; showAlert('Medication updated!'); setShowEditMedicationModal(false); setMedicationToEdit(null); } catch (e) { showAlert(`Update medication error: ${e.message}`, 'error');} }, [supabase, showAlert]);
  const deleteMedicationFromSupabase = useCallback(async (medicationId) => { if (!supabase || !window.confirm("Delete medication?")) return; try { const { error } = await supabase.from('medications').delete().eq('id', medicationId); if (error) throw error; showAlert('Medication deleted!'); } catch (e) { showAlert(`Delete medication error: ${e.message}`, 'error'); } }, [supabase, showAlert]);
  ...
  }, [supabase, showAlert, currentUser]);
  ...
  const updateAnnouncementInSupabase = useCallback(async (announcementData) => {
    if (!supabase || !announcementData.id) return;
  ...
  const deleteAnnouncementFromSupabase = useCallback(async (announcementId) => { if (!supabase || !window.confirm("Delete announcement?")) return; try { const { error } = await supabase.from('announcements').delete().eq('id', announcementId); if (error) throw error; showAlert('Announcement deleted!'); } catch (e) { showAlert(`Delete announcement error: ${e.message}`, 'error'); } }, [supabase, showAlert]);
  ...
  const handleViewInvoiceDetails = useCallback(async (invoice) => {
    ...
    if (childForInvoice && childForInvoice.primary_parent_id) {
        ...
        setParentDetailsForInvoice(null); 
        if (childForInvoice && !childForInvoice.primary_parent_id) { 
            // showAlert("Child has no primary parent linked for invoice details.", "warning"); 
        } else if (!childForInvoice) { 
            showAlert("Child record not found for invoice.", "error");
        }
    }
    setShowViewInvoiceModal(true);
  }, [supabase, showAlert, children, setInvoiceToView, setShowViewInvoiceModal, setParentDetailsForInvoice, setLoadingData]);
  ...
  const updateParentInSupabase = useCallback(async (updatedParentData) => {
    if (!supabase || !parentToEdit?.id) return;
    try {
  ...
  }, [supabase, showAlert, parentToEdit]);

  const deleteParentFromSupabase = useCallback(async (parentId) => {
    if (!supabase || !window.confirm("Delete parent? This may affect linked children.")) return;
    try {
  ...
  const handleSignUp = async (email, password) => { 
    ...
    try {
        const { data, error } = await supabase.auth.signUp({
            ...
            password,
            options: {
              // Example: Pass metadata if your trigger needs it for profile creation
  ...
  const handleSignIn = async (email, password) => { 
    ...
    try {
        ...
        if(error) throw error; 
        // showAlert('Signin successful!'); // Let processSessionChange handle page navigation
    } catch(e) {
  ...
  const handleSignOut = async () => { 
    ...
    setAuthActionLoading(false);
    // State updates are handled by onAuthStateChange -> processSessionChange
  };
    ...
    <AppStateContext.Provider value={{
      ...
      <div className={`app-container ${isSidebarOpen && session && appMode !== 'auth' && !['unknown', 'unknown_profile', 'no_parent_profile', 'exception_profile'].includes(appMode) ? 'sidebar-open' : 'sidebar-closed'}`}>
        ...
        <footer className="app-footer" style={{ 
            ...
            borderTop: '1px solid #dee2e6', // A light top border
            backgroundColor: '#f8f9fa', // A very light background
            marginTop: 'auto' // Pushes footer to bottom if main content is short and app-container is flex column
        }}>
